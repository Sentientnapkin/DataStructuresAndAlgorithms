...
public class Node implements Comparable<Node>{
    ...
    public static int[] countChars(String str) {
        ...
        str.chars()
                .forEach((c) -> freqArr[charToIndex((char)c)]++);
        return freqArr;
    }
    ...
    public static Node createHuffmanTree(int[] counts) {
        ...
        for(int i = 0;i<counts.length;i++){
            if(counts[i]>0) {
                pq.offer(new Node(indexToChar(i), counts[i]));
            }
        }
    ...
    public static void createCodeRec(Node root, String path, String[] table) {
        if(root.ch!='*'){
            table[charToIndex(root.ch)]=path;
        }
        else {
    ...
    public static String decode(String bin, String[] code) {
        String[] encoded = bin.split(" ");
        String decoded = "";

        for(String encodedStr:encoded){
            char c = ' ';
            for(int i = 0;i<code.length;i++){
                if(code[i]!=null && code[i].equals(encodedStr)){
                    c=indexToChar(i);
                }
            }
            decoded+=c;
    ...
    public static int calculateSavings(String message) {
        int normalSize = message.length()*8;
        int[] counts = countChars(message);
        Node root = createHuffmanTree(counts);
        String[] code = createCode(root);
        String encoded = encode(message, code);
        encoded = encoded.replaceAll(" ", "");
        int encodedSize = encoded.length();
        return normalSize-encodedSize;
    }
