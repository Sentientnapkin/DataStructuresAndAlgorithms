...
public class DijkstraGraph {
  ...
  public HashMap<String, Integer> dijkstra(String source) {
    ...
    for (String vertex : vertices.keySet()) {
      distances.put(vertices.get(vertex).label, Integer.MAX_VALUE);
    }
    distances.put(vertices.get(source).label, 0);

    while (!frontier.isEmpty()) {
      int smallestDistance = Integer.MAX_VALUE;
      WeightedVertex smallestDistanceVertex = frontier.get(frontier.size() - 1);
      for (WeightedVertex vertex : frontier) {
        if (distances.get(vertex.label) < smallestDistance) {
          smallestDistanceVertex = vertex;
          smallestDistance = distances.get(vertex.label);
        }
      }

      frontier.remove(smallestDistanceVertex);
      completed.add(smallestDistanceVertex);
      for (DijkstraEdge e : smallestDistanceVertex.edges) {
        WeightedVertex n = e.destination;

        if (distances.get(n.label) > distances.get(smallestDistanceVertex.label) + e.weight) {
          distances.put(n.label, distances.get(smallestDistanceVertex.label) + e.weight);
        }

        if (!(frontier.contains(n) || completed.contains(n))) {
          frontier.add(n);
        }
      }
