...
public class BestFirstSearchGraph {
  ...
  public int bestFirstSearch(String source, String destination) {
    ...
    frontier.offer(new PQNode(source, manhattanDistance(source, destination)));
    HashSet<GridVertex> visited = new HashSet<>();
    visited.add(vertices.get(source));
    ...
    while (!frontier.isEmpty()) {
      ...
      for (WeightedEdge e : v.edges) {
        ...
        if (!(queueContainsLabel(frontier, n.label) || visited.contains(n))) {
          visited.add(n);
          frontier.offer(new PQNode(n.label, manhattanDistance(n.label, destination)));
        }
      }
    }
