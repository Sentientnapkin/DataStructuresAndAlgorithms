...
public class SpanningTreeGraph {
    ...
    public SpanningTreeGraph prim(String source) {
        SpanningTreeGraph spanningTree = createEmptyGraph();
        HashSet<String> connected = new HashSet<>();
        connected.add(source);
        PriorityQueue<WeightedEdge> frontier = new PriorityQueue<>();
        WeightedVertex start = vertices.get(source);
        for(WeightedEdge e: start.edges){
            frontier.offer(e);
        }

        while(!frontier.isEmpty()) {
            WeightedEdge e = frontier.poll();
            if(!connected.contains(e.destination.label)){
                spanningTree.addEdge(e);
                connected.add(e.destination.label);
                for(WeightedEdge extraEdge: e.destination.edges){
                    frontier.offer(extraEdge);
                }
            }
        }
...
//    Sort all of the graph edges by increasing weight
//    While there are edges we haven’t considered yet:
//    Get the next minimum weight edge
//    If adding the edge to our spanning tree doesn’t create a cycle:
//    Add that edge to our spanning tree
    public SpanningTreeGraph kruskal() {
        SpanningTreeGraph spanningTree = createEmptyGraph();
        LinkedList<WeightedEdge> allEdges = getAllEdges();
        Collections.sort(allEdges);

        while(!allEdges.isEmpty()){
            WeightedEdge e = allEdges.get(0);
            spanningTree.addEdge(e);

            if(spanningTree.hasCycle(e.source.label)){
               spanningTree.removeEdge(e);
            }
            allEdges.remove(0);
        }
