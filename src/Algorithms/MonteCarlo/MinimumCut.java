...
public class MinimumCut {
    ...
    public static int karger(WeighedGraph g) {
        int smallestMinimumCut = Integer.MAX_VALUE;
        for (int i = 0; i < 100; i++) {
            smallestMinimumCut = Math.min(smallestMinimumCut, contractionAlgorithm(g));
        }
        return smallestMinimumCut;
    }
...
class WeighedGraph {
    ...
    public void edgeContraction(String label1, String label2) {
        ...
        String combinedLabel = label1 + label2;
        WeighedVertex combined = new WeighedVertex(combinedLabel);
        vertices.put(combinedLabel, combined);
        removeEdge(v1.label, v2.label);
        
        for(int i = 0; i < v1.edges.size();) {
            WeighedEdge e = v1.edges.get(i);
            WeighedVertex neighbor = e.destination;
            if (getEdge(v2.label, neighbor.label) != null) {
                addEdge(combined.label, neighbor.label, getEdge(v2.label, neighbor.label).weight + e.weight);
                removeEdge(v2.label, neighbor.label);
            } else {
                addEdge(combined.label, neighbor.label, e.weight);
            }
            removeEdge(v1.label, neighbor.label);
        }

        for(int i = 0; i < v2.edges.size();) {
            WeighedEdge e = v2.edges.get(i);
            WeighedVertex neighbor = e.destination;
            if (getEdge(combined.label, neighbor.label) == null) {
                addEdge(combined.label, neighbor.label, e.weight);
                removeEdge(v2.label, neighbor.label);
            }
        }
        
        removeVertex(v1.label);
        removeVertex(v2.label);
    }
    ...
    public void addEdge(String label1, String label2, int weight) {
        ...
        if (vertices.containsKey(label1) && vertices.containsKey(label2) && getEdge(label1, label2) == null) {
            WeighedVertex v1 = vertices.get(label1);
            WeighedVertex v2 = vertices.get(label2);
