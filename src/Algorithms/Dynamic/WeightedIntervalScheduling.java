...
public class WeightedIntervalScheduling {
    ...
    public int getOptimalSolution() {
        Integer[] solutions = new Integer[intervals.length];

		solutions[0] = intervals[0].value;

		for(int i = 1; i < solutions.length; i++){
			int outOfBoundsChecker = 0;
			if(latestNonOverlapping(i) >= 0){
				outOfBoundsChecker = solutions[latestNonOverlapping(i)];
			}

			int choose = intervals[i].value + outOfBoundsChecker;
			int notChoose = solutions[i-1];
			solutions[i] = Math.max(choose, notChoose);
		}
